...
from src.reporting.infographic_presentation_generator import InfographicPresentationGenerator
import logging
...
class ReportBuilder:
    def __init__(self, report_month: str = None):
        ...
        if report_month:
            ...
            today = datetime.now()
            self.report_month = (today.replace(day=1) -
    ...
    def build_report(self, include_voice: bool = True):
        """Generate all reports for the specified month.

        Args:
            include_voice: Whether to generate voice narration (default: True)
        """
        ...
        try:
            ...
            # Generate LLM-based executive summary with infographics
            self._build_executive_summary()

            # Generate voice narration
            if include_voice:
                logger.info("Generating voice narration...")
                audio_path = self.generate_voice_narration()
                if audio_path:
                    logger.info(f"Voice narration saved to: {audio_path}")
    ...
    def _build_sales_channel_report(self):
        ...
        self._render_template('sales_channel_report.j2',
                              context, 'sales_channel_report.html')
    ...
    def _build_product_performance_report(self):
        ...
        self._render_template('product_performance_report.j2',
                              context, 'product_performance_report.html')
    ...
    def _build_inventory_health_report(self):
        ...
        self._render_template('inventory_health_report.j2',
                              context, 'inventory_health_report.html')
    ...
    def _build_customer_demographics_report(self):
        ...
        avg_latency = data['purchase_latency']['days_to_first_purchase'].mean(
        ) if not data['purchase_latency'].empty else 0

        # Collect data for LLM
        self.aggregated_data['customer_summary'] = {
            'avg_latency': round(avg_latency, 1),
            'new_customers': len(data['new_customers']) if not data['new_customers'].empty else 0,
            'top_states': data['geographic_distribution'].head(5).to_dict(orient='records')
        }
        ...
        self._render_template('customer_demographics_report.j2',
                              context, 'customer_demographics_report.html')
    ...
    def _build_payment_analysis_report(self):
        ...
        # Collect data for LLM
        self.aggregated_data['payment_summary'] = {
            'total_amount': data['payment_methods']['total_amount'].sum(),
            'unsettled_count': len(data['unsettled_orders']),
        ...
        self._render_template('payment_analysis_report.j2',
                              context, 'payment_analysis_report.html')
    ...
    def _build_executive_summary(self):
        """Generate LLM-based executive summary PowerPoint with infographics."""
        logger.info("Generating Executive Summary with LLM and Infographics...")
        ...
        try:
            ...
            # Store insights for voice generation
            self.insights = insights

            # Generate PowerPoint with AI-generated infographics
            pptx_generator = InfographicPresentationGenerator()
            pptx_path = self.output_dir / 'executive_summary.pptx'
            infographic_dir = self.output_dir / 'infographics'

            pptx_generator.generate_presentation(
                insights=insights,
                report_data=self.aggregated_data,
                output_path=str(pptx_path),
                infographic_dir=str(infographic_dir)
            )

            logger.info(
                f"Executive summary PowerPoint with infographics created: {pptx_path}")

        except Exception as e:
            logger.error(f"Error generating executive summary: {e}")
            logger.warning("Continuing without executive summary...")

    def generate_voice_narration(self) -> str:
        """
        Generate voice narration audio for the executive summary.
        Requires ElevenLabs API key to be configured.
        ...
        if not settings.ELEVENLABS_API_KEY:
            logger.warning(
                "ElevenLabs API key not configured. Skipping voice narration.")
            return None

        try:
            from src.voice import VoiceReportService

            # Detect LLM provider
            provider = 'gemini' if settings.LLM_API_KEY.startswith(
                'AIza') else 'openai'
            ...
            # Use stored insights or generate new ones
            ...
            month_slug = self.report_month.strftime('%Y_%m')
            audio_filename = f"executive_summary_{month_slug}.mp3"

            result = voice_service.generate_voice_report(
                insights=insights,
                report_data=self.aggregated_data,
            ...
            if result["success"]:
                logger.info(
                    f"Voice narration generated: {result['audio_path']}")
                return result["audio_path"]
            else:
                logger.error(f"Voice narration failed: {result.get('error')}")
                return None
